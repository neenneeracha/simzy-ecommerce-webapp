...
const addNewUser = (req, res) => {
  ...
  pool.query(q, req.body.email, (err, data) => {
    ...
    q =
      "INSERT INTO userinfo (`email`,`password`,`name`,`surname`,`gender`,`phone_number`,`address`,`district`,`province`,`zip_code`) VALUES (?)";
    ...
    if (data.length > 0) {
      ...
      if (isEqual) {
        const token = jwt.sign(
          { user_id: data[0].user_id, is_admin: data[0].is_admin },
        ...
        return res
          .status(403)
          .json({ msg: `Incorrect password for user with email: ${email} !!` });
    } else
      return res
        .status(404)
