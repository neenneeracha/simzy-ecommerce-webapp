...
const getAllUserInfo = (req, res) => {
  const q =
    "SELECT user_id, is_admin, name, surname, gender, email, phone_number, address, district, province, zip_code, created_at, updated_at FROM userinfo";
...
const updatePassword = (req, res) => {
  ...
  pool.query(q, [user_id], (err, data) => {
    ...
    q =
      "UPDATE userinfo SET password = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?";
...
const updateUserInfo = (req, res) => {
  ...
  let q =
    "SELECT EXISTS (SELECT * FROM userinfo WHERE email = ? AND user_id != ?) AS userExist";
  ...
  pool.query(q, [req.body.email, user_id], (err, data) => {
    ...
    q =
      "UPDATE userinfo SET email = ?, name = ?, surname = ?, gender = ?, phone_number = ?, address = ?, district = ?, province = ?, zip_code = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?";
...
const updateInfoByAdmin = (req, res) => {
  ...
  pool.query(q, [req.body.user.email, user_id], (err, data) => {
    ...
    q =
      "UPDATE userinfo SET email = ?, name = ?, surname = ?, gender = ?, phone_number = ?, address = ?, district = ?, province = ?, zip_code = ?, is_admin = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?";
  ...
  pool.query(q, req.body.user.email, (err, data) => {
    ...
    if (data[0].userExist === 1)
      return res.status(409).json({
        msg: `User with email: ${req.body.user.email} already exist !!`,
      });
    ...
    pool.query(q, [values], (err, data) => {
      ...
      return res
        .status(201)
        .json({ msg: `User created with User ID: ${data.insertId} !!` });
    });
  });
};
...
const removeUser = (req, res) => {
  const user_id = req.params.id;
  let q =
    "SELECT EXISTS (SELECT * FROM productorder WHERE user_id = ? AND (status_id = 1 OR status_id = 2)) AS orderExist";
  ...
  pool.query(q, [user_id], (err, data) => {
    ...
    if (data[0].orderExist === 1)
      return res.status(409).json({
        msg: `User #${user_id} has pending orders, can't be deleted now !!`,
      });

    if (user_id === "1")
      return res.status(409).json({ msg: `Master admin can't be deleted !!` });
    ...
    pool.query(q, [user_id], (err, data) => {
      ...
      return res
        .status(202)
        .json({ msg: `User #${user_id} has been deleted !!` });
    });
  });
};
