...
const removeProduct = (req, res) => {
  const product_id = req.params.id;
  let q =
    "SELECT EXISTS (SELECT * FROM orderhistory oh LEFT JOIN productstock ps ON oh.stock_id = ps.stock_id LEFT JOIN productcolor pc ON ps.product_color_id = pc.product_color_id WHERE product_id = ?) AS productOrdered";
  ...
  pool.query(q, [product_id], (err, data) => {
    ...
    if (data[0].productOrdered === 1)
      return res
        .status(409)
        .json({
          msg: `Product #${product_id} has already been ordered, can't be deleted !!`,
        });
    ...
    pool.query(q, [product_id], (err, data) => {
      ...
      return res
        .status(202)
        .json({ msg: `Product #${product_id} has been deleted !!` });
    });
  });
};
...
const addNewProduct = (req, res) => {
  ...
  const q =
    "INSERT INTO product (`category_id`, `product_name`, `description`, `details`, `price`) VALUES (?)";
