...
const UpdateStockColor = (req, res) => {
  ...
  const q =
    "SELECT EXISTS (SELECT * FROM colorgroup WHERE color_group_id = ? AND color = ?) AS colorExist";
  const q1 =
    "SELECT EXISTS (SELECT * FROM productcolor pc WHERE product_id = ? AND color_group_id = ?) AS colorExist";
  const q2 =
    "UPDATE productcolor SET color_group_id = ? WHERE product_color_id = ?";
    ...
    pool.query(q, [colors[i].color_group_id, colors[i].color], (err, data) => {
      ...
      if (status !== 200) {
        ...
        if (data[0].colorExist !== 1) {
          ...
          pool.query(
            q1,
            [product_id, colors[i].color_group_id],
            (err, data) => {
              if (err) {
                status = 500;
                error = err;
              ...
              if (status !== 200) {
                ...
                if (data[0].colorExist === 0) {
                  ...
                  pool.query(
                    q2,
                    [colors[i].color_group_id, colors[i].product_color_id],
                    (err) => {
                      if (err) {
                        status = 500;
                        error = err;
